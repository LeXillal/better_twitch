...
function getOnlineStreams() {
    return Array.from(document.querySelectorAll(".dpqRKW")).filter(stream => !Boolean(stream.querySelector(".side-nav-card__avatar--offline")));
}
...
function handleHover(stream) {
    ...
    const linkStream = stream.querySelector("a").getAttribute("href").replace("/", "");
...
async function main() {
    ...
    await wait(1000) // On attend 1 seconde pour que la page soit chargée puis pour boucler
